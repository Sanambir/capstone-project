...
def get_agent_id():
    ...
    if os.path.exists(agent_file):
        ...
        # Generate a temporary UUID; this value will be replaced by the auto-generated ID from JSON Server.
...
def create_vm_record(initial_data):
    ...
    JSON Server will auto-generate an ID.
    """
    try:
...
def ensure_vm_record():
    ...
    If not (i.e. GET returns 404), create a new record (without specifying an ID).
    After creation, update our local agent_id and API_URL based on the auto-generated ID.
    """
    ...
    try:
        ...
        if get_response.status_code == 404:
            # Record not found; create a new record without providing an "id".
            initial_data = {
                "name": host_name,
                "os": os_type,
                "cpu": 0,
                ...
                "status": "Running",
                # Use UTC for the timestamp and append "Z" to denote UTC
                "last_updated": datetime.datetime.utcnow().isoformat() + "Z"
            }
            created_record = create_vm_record(initial_data)
            if created_record:
                # Use the ID auto-generated by JSON Server.
                new_id = str(created_record.get("id"))
                if new_id != agent_id:
                    agent_id = new_id
...
def collect_metrics():
    ...
    """
    metrics = {
        "id": agent_id,         # Use the updated agent_id
